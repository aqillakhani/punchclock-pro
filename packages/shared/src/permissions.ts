...
export const PERMISSIONS = {
  ...
  // Time-entry corrections. Workers request; managers/owners decide.
  // EDIT_TIME_ENTRY is the stronger grant: change a record directly,
  // without a request, which is why employees never hold it.
  VIEW_TIME_CORRECTION: 'view:time-correction',
  SUBMIT_TIME_CORRECTION: 'submit:time-correction',
  APPROVE_TIME_CORRECTION: 'approve:time-correction',
...
const EMPLOYEE_PERMISSIONS: ReadonlySet<Action> = new Set<Action>([
  ...
  PERMISSIONS.VIEW_TIME_OFF,
  PERMISSIONS.SUBMIT_TIME_OFF,
  // Employees may ask for a fix, but never apply one themselves.
  PERMISSIONS.VIEW_TIME_CORRECTION,
  PERMISSIONS.SUBMIT_TIME_CORRECTION,
  PERMISSIONS.VIEW_TRADES,
  PERMISSIONS.POST_TRADE,
